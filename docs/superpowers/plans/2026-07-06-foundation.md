...
- **Plan 2 — SIRIL preprocess core (§4.4a):** the parameterized `preprocess` (link → calibrate → register → stack → [spcc] → golden anchor), `.ssf` generation into `logs/`, `seqplatesolve` false-negative handling (FR-26), stage execution wired into the manifest with `--from`/`--redo`/`--continue`.
  - **Reconcile before wiring stages (final-review finding #9):** `preflight.MODE_TOOLS` requires `starnet2` for `dso-reflection-nebula`, but `cli.MODE_ORDER` has no `starnet` stage for it (only `dso-mosaic` does). Decide: give reflection an explicit `starnet` stage in `MODE_ORDER`, or have its Python dual-layer `finish` absorb star handling (and drop the starnet2 preflight requirement if `finish` calls StarNet2 internally). The two per-mode tables must agree — they are the resume/golden-anchor backbone (FR-24d). Also revisit the deferred foundation Minors: config value-type validation, `input_fingerprint` sub-second mtime, bare `list`/`dict` type hints, and `siril_config_dir`/env-fallback test coverage.
